...
const rarities = [{
  "traits": {
    ...
    "Clothes": {
      ...
      "Coats (Wine/Green/White/Black/Blue)": "10.17%",
      "Hoodies(Blue/Purple/Pink/DBlue/Red)": "11.58%",
      "Pimp Clothes (Luxuy/Orange/Purple/Pink)": "21.82%",
      "None": "10.67%",
      "Vest": "7.98%",
      "Black smoking": "7.30%",
      "Blue smoking": "6.73%",
...
explorerApiV1 = 'https://api.ergoplatform.com/api/v1'
 ...
 function scrollToTop() {
  ...
  function buildPage() {
    ...
    if (nftEndIndex > auctions.length - 1) {
      nftEndIndex = auctions.length - 1;
    }
    for(let i = nftStartIndex; i < nftEndIndex; i++) {
      console.log(nftStartIndex + " -> " + nftEndIndex);
    ...
    nftStartIndex = nftEndIndex + 2;
    nftEndIndex += 2;
  }

  // Load the next section of NFTs
  function loadMore() {
    buildPage(nftIndex, nftIndex + 2);
  }
